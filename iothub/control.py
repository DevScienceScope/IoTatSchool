...
camera =       settings["configuration"]["tech"]["camera"]
...
if logger == enabled:
    ...
    #Send it command to execute in the script location
    os.system("screen -r thermal -p0 -X stuff 'cd /home/pi/IoTatSchool/iothub; sh thermal.sh'")
    #Send carriage return to execute
    os.system("screen -r thermal -p0 -X eval 'stuff \015'")

elif camera == enabled:
    #Create login session
    os.system("screen -dmS camera")
    #Send it command to execute in the script location
    os.system("screen -r camera -p0 -X stuff 'cd /home/pi/IoTatSchool/iothub; sh cameraControl.sh'")
    #Send carriage return to execute
    os.system("screen -r camera -p0 -X eval 'stuff \015'")